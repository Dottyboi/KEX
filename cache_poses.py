...
def cache_one(pose_path: Path, cache_path: Path) -> bool:
    ...
    try:
        ...
        left_hand  = _masked_to_float(data_3d[:, _LEFT_HAND_SLICE,    :], conf_2d[:, _LEFT_HAND_SLICE])
        right_hand = _masked_to_float(data_3d[:, _RIGHT_HAND_SLICE,   :], conf_2d[:, _RIGHT_HAND_SLICE])
        body       = _masked_to_float(data_3d[:, _UPPER_BODY_INDICES, :], conf_2d[:, _UPPER_BODY_INDICES])
...
def main():
    ...
    parser.add_argument("--pose_dir",  default="/data/ASLLVD/pose",       type=Path)
    parser.add_argument("--cache_dir", default="/data/ASLLVD/pose_cache", type=Path)
    ...
    pose_files = sorted(args.pose_dir.glob("*.pose"))
    print(f"Found {len(pose_files)} .pose files in {args.pose_dir}")

    ok = skip = fail = 0
    for i, pose_path in enumerate(pose_files, 1):
        cache_path = args.cache_dir / (pose_path.name + ".npz")
        if cache_path.exists() and not args.overwrite:
            skip += 1
            if i % 500 == 0:
                print(f"  [{i}/{len(pose_files)}] {skip} skipped, {ok} cached, {fail} failed")
            continue
    ...
    if fail:
        print(f"Failed files will fall back to live pose_format loading at training time.")
